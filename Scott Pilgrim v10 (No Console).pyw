...
opnl = []
for i in range(0,20):
    imglid = "opnl (" + str(i + 1) + ").gif"
    opnl.append(pygame.image.load(os.path.join("Images", "opnl", imglid)))
...
def updateevil():
    ...
    if nega.status == 1:
        ...
        fontt = pygame.font.SysFont("impact", 27)
        txt = font.render(str(nega.hp), 1, (255, 255, 255))
        txtout = fontt.render(str(nega.hp), 1, (0, 0, 0))
        screen.blit(txtout, (nega.x + 18, nega.y - 15))
        screen.blit(txt, (nega.x + 18, nega.y - 15))
...
class evil(object):
    ...
    def detecthit(self):
        if self.status == 1:
            ...
            # print (dist)
            if scott.getstatus() == "a" and dist < 40 and self.at == 0:
                self.hp -= 10
                self.at = 1
            else:
